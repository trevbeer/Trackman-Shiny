...
## taiyo
total <- read_csv('~/UCLA/Baseball/Trev_Shiny/allGames.csv')
# removing unnecessary tags, pitchouts etc
total <- total %>% subset(!TaggedPitchType %in% c("Other", "Knuckleball"))
...
# create counts
total$Date <- as.Date(total$Date, "%m/%d/%Y") # this is just duplicate yeah? delete?
...
total$ExitSpeed <- ifelse(total$TaggedHitType == "Bunt", NA, total$ExitSpeed)
total$Angle <- ifelse(total$TaggedHitType == "Bunt", NA, total$Angle)
